...
;;; Derick Eddington, on 2008-01-03, did:
;;;  Changed to use case-lambda for optional arguments.
;;;  Changed to use R6RS assertion-violation for pregexp-error.
;;;  Fix bug in pregexp-replace* which caused infinte loop
;;;   when the regexp matches a zero-length span.
;;;  Fix bug in pregexp-replace* when the regexp matches a zero-length span
;;;   at the beginning of its search span. Now the replacement (for the 
;;;   zero-span match) is inserted inbetween the surrounding characters, 
;;;   instead of incorrectly ommiting them.
;;;  Fix bug in expand-time known-string special cases optimization.
;;;  Some optimizations for replacing-functions so they don't do unneeded allocation.
;;;  Added pregexp-match-positions* and pregexp-match*
;;;   which are like their non-* counterparts except they
...
;(define *pregexp-nul-char-int* #\x0) ; not used
...
(define pregexp-error
  ...
  (case-lambda
    [(who) 
     (assertion-violation who "an error occurred")]
    [(who msg . args) 
     (apply assertion-violation who msg args)]))
...
(define-regexp-proc pregexp-match-positions
  (case-lambda
    [(pat str)
     (pregexp-match-positions pat str 0 (string-length str))]
    [(pat str start)
...
(define-regexp-proc pregexp-match-positions*
  (case-lambda
    ...
    [(pat str start)
     (pregexp-match-positions* pat str start (string-length str))]
    [(pat str start end)
     (when (string? pat) (set! pat (pregexp pat)))
     (let loop ([i start] [r '()])
       (if (>= i end)
         (reverse r)
         (let ([pp (pregexp-match-positions pat str i end)])
           (if pp
             (loop (cdar pp) (cons pp r))
             (reverse r)))))]))
...
(define-regexp-proc pregexp-match
  (case-lambda
    [(pat str)
     (pregexp-match pat str 0 (string-length str))]
    [(pat str start)
     (pregexp-match pat str start (string-length str))]
    [(pat str start end)
          ...
          (let ((pp (pregexp-match-positions pat str i n)))
            (if (not pp) 
              (if (= i 0)
                ...
                (apply string-append (reverse (cons (substring str i n) r))))
              (let ([ps (caar pp)] [pe (cdar pp)])
                (loop (if (= ps pe) 
                        (+ 1 pe) ;; otherwise we'll find the same match and infinitely loop
                        pe)
                      pe
                      (cons* (pregexp-replace-aux str ins ins-len pp)
                             (substring str lag ps)
                             r))))))))))
