...
(let ([lib+ids 
  ...
  (for-each (lambda (l+ids)
              ...
              (for-each (lambda (id)
                        ...
                        (cdr l+ids)))
            lib+ids)
  ;;; makefile's identifiers in an R6RS library but not in (library (ikarus))
  (printf "makefile R6RS identifiers not in (ikarus):\n")
  (for-each (lambda (l+ids)
              (define lib-ikarus (LN->CN '(ikarus)))
              (for-each (lambda (id)
                          (cond [(assoc id makefile/identifier->library-map)
                                 => (lambda (p)
                                      (define cns (cdr p))
                                      (unless (member lib-ikarus cns)
                                        (printf " ~s  from ~s\n" id (car l+ids))
                                        (printf "  not in ~s -> ~s\n" '(ikarus) lib-ikarus)))]))
                        (cdr l+ids)))
            lib+ids))
