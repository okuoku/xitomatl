...
(define-syntax check-finished
  (syntax-rules ()
    [(_ expr) 
     (check (guard (ex [else (&coroutine-finished? ex)])
              expr
              'unexpected-return)
            => #t)]))
...
(define-coroutine (g7)
  (yield 1) 
  (yield 2))
(check (guard (ex [(&coroutine-finished? ex) 'first-dyn-env])
         (g7))
       => 1)
(check (guard (ex [(&coroutine-finished? ex) 'second-dyn-env])
         (g7))
       => 2)
(check (guard (ex [(&coroutine-finished? ex) 'third-dyn-env])
         (g7))
       => 'third-dyn-env)
(define-coroutine (g8)
  (yield 1)
